...
* Inventory Integration – the orchestrator’s Inventory Database is extended with new model for session
  ...
  * New API Endpoint – an orchestrator API that allows operators to initiate, monitor, and terminate
    sessions via UI or terminal tools that uses websocket.
        ...
        The architectural separation between the Proxy Manager and the Remote Access Proxy is a deliberate design choice that
  enables flexible, independent scaling of the control plane and data plane according to system load.
...
2. Agent ↔ Remote Access Proxy
   ...
   By default, the Agent only possesses a public SSH key, which is insufficient for establishing a secure SSH session.
   Therefore, an additional mechanism is required to enable authenticated communication with the Remote Access Proxy.
   <br></br>
   Two possible approaches are considered:
    - Ephemeral certificates — dynamically issued short-lived certificates signed by a trusted authority.
    - SSH keys — pre-generated key pairs with secure key distribution and management.
    
   The communication channel will be established using SSH over WebSocket. 
   <br></br>
...
```mermaid
sequenceDiagram
title Agent Onboarding & Tunnel Bring-up (Vault-integrated)

    participant AG as Agent (Edge Node)
    participant RAM as Remote Access Manager (Control Plane)
    participant VAULT as Vault (PKI / CA)
    participant RAP as Remote Access Proxy (Data Plane)

    rect rgb(245,245,245)
    note over AG,RAM: Device enrollment / attestation
    AG->>RAM: Device attestation + enroll request
    RAM->>VAULT: Issue device mTLS cert (role=device, TTL≈30d)
    VAULT-->>RAM: X.509 device cert + CA chain
    RAM-->>AG: Deliver device cert + CA chain (provisioning channel)
    end

    AG->>RAP: Establish mTLS (present device X.509 cert)
    note over RAP: Device identity pinned to Vault CA

    rect rgb(245,245,245)
    note over AG,RAM: Ephemeral SSH cert for tunnel
    AG->>RAM: Request OpenSSH cert (role=agent-tunnel)
    RAM->>VAULT: Sign SSH cert (TTL≈10m, critical options)
    VAULT-->>RAM: OpenSSH cert
    RAM-->>AG: OpenSSH cert
    end

    AG->>RAP: Reverse SSH bring-up (SSH over WebSocket/mTLS)
    RAP->>RAM: (Optional) Register tunnel {session_id, device_id}
    RAM-->>RAP: Ack / policy context
```


```mermaid
sequenceDiagram
    title User Session Attach via Remote Access Proxy (Vault + Session Grant)
    ...
    participant VAULT as Vault (PKI / CA)
    participant RAP as Remote Access Proxy (Data Plane)
    participant AG as Agent (Reverse SSH up)

    U->>UI: Click "Connect to device"
    UI->>RAM: Request session for device_id

    rect rgb(245,245,245)
    note over RAM: AuthN (SSO) + AuthZ (RBAC/ABAC, policy)
    RAM->>VAULT: Issue ephemeral user cert (SSH/X.509, TTL≈5m)
    VAULT-->>RAM: User cert
    RAM-->>UI: Session Grant (SG, one-time, TTL≈60–120s)<br/>+ optional user cert
    RAM->>RAP: Backchannel register SG (hash/jti, session_id) via mTLS
    end

    alt DPoP / Proof-of-Possession enabled
        UI->>RAP: WSS upgrade + SG (+ DPoP proof)
        RAP->>RAM: Verify SG (sig/JWKS or introspection) + consume jti
        note over RAP: Enforce audience, device_id, session_id, exp, one_time
    else Basic SG
        UI->>RAP: WSS upgrade + SG
        RAP->>RAM: Verify SG (sig/JWKS or introspection) + consume jti
    end

    RAP->>AG: Attach to reverse SSH (session_id)
    AG-->>RAP: PTY stream
    RAP-->>UI: SSH over WebSocket (interactive session)

    rect rgb(245,245,245)
    note over RAM,VAULT: Revocation & expiry
    RAM->>RAP: revoke(session_id) → drop channel (if needed)
    VAULT->>RAP: Cert TTL expiry (user/agent) → session ends
    end
```
